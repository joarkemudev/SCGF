<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@taglib prefix="tiles" uri="http://tiles.apache.org/tags-tiles"%>
<%@taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
<%@taglib prefix="form" uri="http://www.springframework.org/tags/form"%>
<%@taglib prefix="sec" uri="http://www.springframework.org/security/tags"%>
<tiles:insertDefinition name="defaultTemplate">
<fmt:setLocale value="en_US" scope="session"/>

	<tiles:putAttribute name="title">
		<fmt:message key="catalogos.empresas" />
	</tiles:putAttribute>

	<tiles:putAttribute name="body">
		<div class="row">

			<div class="col-md-12">
				<div class="card">
					<div class="card-header card-header-icon"
						data-background-color="blue">
						<i class="material-icons">domain</i>
					</div>
					<div class="card-content">
						<h4 class="card-title">Empresas</h4>
						
						<div class="toolbar">
							<sec:authorize access="hasAnyRole('ALTA_EMPRESAS')">
							<a href='<c:url value="add"/>' class="btn btn-success" data-toggle="tooltip" 
								title="Crea una nueva Empresa"> <i
								class="fa fa-plus"></i> <span class="hidden-480"> Nuevo </span>
							</a> 
							</sec:authorize>
							
						</div>
						
						<hr>
						<div class="toolbar">

							<label class="control-label"></label>
	
						</div>
						<table id="tableEmpresas" class="table table-striped table-no-bordered table-hover ">
							<thead>
								<tr>
									<th>Id</th>
									<th><fmt:message key="catalogos.empresas.nombre" /></th>
									<th>Representante Legal</th>
 									<th><fmt:message key="catalogos.empresas.nit" /></th> 
 									<th><fmt:message key="catalogos.usuarios.correoElectronico" /></th>
									<th>Telefono</th>
									<th>Logo</th>
									<sec:authorize access="hasAnyRole('ELIMINA_EMPRESAS')">
									<th><fmt:message key="catalogos.acciones" /></th>
									</sec:authorize>
								</tr>
							</thead>
							<tbody>
								<c:forEach var="emp" items="${empresas}">
									<tr>
										<td>${emp.id}</td>
										<td>
									    <sec:authorize access="hasAnyRole('EDITA_EMPRESAS')">
									        <!-- Si el usuario tiene permiso, muestra el nombre como un enlace -->
									        <a href="edit?id=${emp.id}">${emp.nombre}</a>
									    </sec:authorize>
									    <sec:authorize access="!hasAnyRole('EDITA_EMPRESAS')">
									        <!-- Si el usuario no tiene permiso, muestra solo el nombre como texto -->
									        ${emp.nombre}
									    </sec:authorize>
										</td>
										<td>${emp.responsable}</td>
 	                                   	<td>${emp.nit}</td>
 	                                   	<td>${emp.correoElectronico}</td>	                                   
 	                                   	<td>${emp.telefono}</td> 
 	                                   	<td><img src="${pageContext.request.contextPath}/assets/img/${emp.rutaImagen}" class="logo-img"/></td>
 	                                   	<sec:authorize access="hasAnyRole('ELIMINA_EMPRESAS')">
 	                                   	<td>
 										<button type="button" onclick="eliminar('delete?id=${emp.id}');" class="btn btn-xs btn-danger btn-simple"><i class="fa fa-trash-o"></i></button>
 										</td>
 										</sec:authorize>
									</tr>
								</c:forEach>
							</tbody>

						</table>

					</div>
				</div>
			</div>

		</div>
	</tiles:putAttribute>

	<tiles:putAttribute name="ready"> 
	$('#empresas_op').addClass("active ");
	$('#empresas').addClass("collapse in");
    
	$('#tableEmpresas').DataTable({
        responsive: true,
        searching: true,
        info:false,
        "language": {"url": "<c:url value="/assets/js/SpanishDT.json"/>"}
    });

   </tiles:putAttribute>

</tiles:insertDefinition>